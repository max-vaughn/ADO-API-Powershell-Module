...
<#
  Create an enum type based on the LinkTypes described in this documentation link:
  https://docs.microsoft.com/en-us/azure/devops/boards/queries/link-type-reference?view=azure-devops
#>
<#
Add a type for the operation action 
...
using System;
 ...
 {   
    public class CreateOp
    {
        ...
        public CreateOp( string operation, string Value, string Path, string From)
        {
            op = operation;
            value = Value;
            path = Path;
...
Add-Type -TypeDefinition @"
    ...
    {
        HierarchyForward = const "System.LinkTypes.Hierarchy-Forward",
        HierarchyReverse = "System.LinkTypes.Hierarchy-Reverse",
        DuplicateForward = "System.LinkTypes.Duplicate-Forward",
        DuplicateReverse = "System.LinkTypes.Duplicate-Reverse",
        Related = "System.LinkTypes.Related",
        AffectsReverse = "Microsoft.VSTS.Common.Affects-Reverse",
        AffectsForward = "Microsoft.VSTS.Common.Affects-Forward"
    }
"@

  Token List:
  IADOCOmm Sandbox 12/18/2020 fyvwzzvrmutdwlme6nsjtyfp6arlqzj7tj4unkz5r4fuxnvp3q7a
...
<#/
Intialiaze-AzureADWiki - initialized base URL and token hash variable with the hashed token and the associated base URL for 
    AzureAD Wiki ( https://supportability.visualstudio.com/AzureAD ) 
    The function returns the header values required to make ADO API calls.
#>
Function Initialize-AzureADWiki {
    param (
        [Parameter()]
        [string] $perTok = "qbdqoodhlnbkaesqh4c27y553foxgq77fxokytckd26flj3zpgaq"
    )
    $global:strPersonalToken = $perTok
    $global:strEncodedPersonalToken = [System.Convert]::ToBase64String( [System.Text.Encoding]::ASCII.GetBytes(":$global:strPersonalToken"))
    $retHash = @{ token = $global:strEncodedPersonalToken; org = $global:strOrgUri }
    $global:gHeaders = Set-ADOAuthHeaders -tokenHash $retHash
    #$global:WikiInfo = Get-Wikis -baseUrl "https://dev.azure.com" -Project AzureAD -wikiName "AzureAD" -headers $global:gHeaders
    $Context = Get-ADOContext -pat $perTok -organization "Supportability" -project "AzureAD"
    $global:WikiInfo = $Context.WikiInfo.value
    $global:strOrgUri = $Context.OrgUrl
    return $Context
}
<#
Initialize-IADOCommSandbox - initialized base URL and token hash variable with the hashed token and the associated base URL for 
    IADOComm Wiki ( https://dev.azure.com/IADOCOmm ) 
    The function returns the header values required to make ADO API calls.
#>
function Initialize-IADOCommSandbox {
    [CmdletBinding()]
    param (
        [Parameter()]
        [string] $perTok = "fyvwzzvrmutdwlme6nsjtyfp6arlqzj7tj4unkz5r4fuxnvp3q7a"
    )
    $global:strPersonalToken = $perTok
    $global:strEncodedPersonalToken = [System.Convert]::ToBase64String( [System.Text.Encoding]::ASCII.GetBytes(":$global:strPersonalToken"))
    $retHash = @{ token = $global:strEncodedPersonalToken; org = $global:strOrgUri }
    $global:gHeaders = Set-ADOAuthHeaders -tokenHash $retHash
    Write-DebugObject -debugString "Initalize-IADOCommSandbox" -inputObject $global:gHeaders -ForegroundColor DarkYellow
    $Context = Get-ADOContext -wikiName "0667c910-c6f1-430a-8310-9db72135b525" -pat $perTok -project "IADOCOmm" -organization "IdentityCommunities"
    $global:WikiInfo = $Context.WikiInfo
    $global:strOrgUri = $Context.OrgUrl
    return $Context
}
function Initialize-IADOComm {
    [CmdletBinding()]
    param (
        [Parameter()]
        [string] $perTok = "aelyc4oilgj7ma5hyngh7bmu4wo3hlwwe6jx4cziiuj654p5jvea"
    )
    $global:strPersonalToken = $perTok
    $global:strEncodedPersonalToken = [System.Convert]::ToBase64String( [System.Text.Encoding]::ASCII.GetBytes(":$global:strPersonalToken"))
    $retHash = @{ token = $global:strEncodedPersonalToken; org = $global:strOrgUri }
    $global:gHeaders = Set-ADOAuthHeaders -tokenHash $retHash
    Write-DebugObject -debugString "Initalize-IADOComm" -inputObject $global:gHeaders -ForegroundColor DarkYellow
    $Context = Get-ADOContext -wikiName "Identity Azure Dev Ops Community Wik" -pat $perTok -organization "IdentityCommunities" -Project "Community - Identity Azure Dev Ops Wiki"
    $global:WikiInfo = $Context.WikiInfo
    $global:strOrgUri = $Context.OrgUrl
    return $Context
}

function Initialize-CommunityTemplate {
    [CmdletBinding()]
    param (
        [Parameter()]
        [string] $perTok = "scqnpcvjwa4tuga5vvjjx7xlfcr3d7ejxtghdxndeynkza4cc57q"
    )
    $global:strPersonalToken = $perTok
    $global:strEncodedPersonalToken = [System.Convert]::ToBase64String( [System.Text.Encoding]::ASCII.GetBytes(":$global:strPersonalToken"))
    $retHash = @{ token = $global:strEncodedPersonalToken; org = $global:strOrgUri }
    $global:gHeaders = Set-ADOAuthHeaders -tokenHash $retHash
    Write-DebugObject -debugString "Initalize-CommunityTemplate" -inputObject $global:gHeaders -ForegroundColor DarkYellow
    $Context = Get-ADOContext -pat $perTok -organization "IdentityCommunities" -Project "Community - Template"
    $global:WikiInfo = $Context.WikiInfo
    $global:strOrgUri = $Context.OrgUrl
    return $Context
}
<#
Initialize-IdentityDevExp - initialized base URL and token hash variable with the hashed token and the associated base URL for 
    IdentitiesCommunity organization and the Community - Identity Developer Experiences project( https://dev.azure.com/IADOCOmm ) 
    The function returns the header values required to make ADO API calls.

    There is no code wiki associated with this project, so there is nothing returned in the wikiInfo global.
#>
function Initialize-IdentityDevExp {
    [CmdletBinding()]
    param (
        [Parameter()]
        [string] $perTok = "33s65txx5lwfwpmjllhxyj6zgp25i5glpy66wybverzgpeuzcxga"
    )
    $global:strPersonalToken = $perTok
    $global:strEncodedPersonalToken = [System.Convert]::ToBase64String( [System.Text.Encoding]::ASCII.GetBytes(":$global:strPersonalToken"))
    ...
    $retHash = @{ token = $global:strEncodedPersonalToken; org = $global:strOrgUri }
    $global:gHeaders = Set-ADOAuthHeaders -tokenHash $retHash
    Write-DebugObject -debugString "Initalize-IdentityDevExp" -inputObject $global:gHeaders -ForegroundColor DarkYellow
    # There is no wiki in the Identity Developer Experiences project. $global:WikiInfo = Get-Wikis -Project "" -wikiName "0667c910-c6f1-430a-8310-9db72135b525" -headers $global:gHeaders
